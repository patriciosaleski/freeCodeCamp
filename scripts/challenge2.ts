const romanConvertButton = document.getElementById("roman-button") as HTMLInputElement
const challenge2Input = document.getElementById("challenge2-input") as HTMLInputElement
const challenge2Result = document.getElementById("challenge2-result") as HTMLInputElement
...
challenge2Input.addEventListener("keyup", () => {
  ...
  value < 1 || value > 3999
    ? (
        ...
        romanConvertButton.setAttribute("title", "Invalid input")
      ) : (
        romanConvertButton.disabled = false,
...
function romanConvert(num: number): string {
  const values = {
    M: 1000,
    CM: 900,
    D: 500,
